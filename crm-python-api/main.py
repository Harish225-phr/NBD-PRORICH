...
async def process_leads(request: FilterRequest):
    ...
    try:
        ...
        print(f"DEBUG: process_leads - user_id={user_id}, role={role}, filter_type={filter_type}, leads_count={len(leads)}")
        
        # Validate leads data
        if not isinstance(leads, list) or len(leads) == 0:
            print("DEBUG: No leads provided")
            return {
                "success": True,
                "count": 0,
                "leads": []
...
async def dashboard_stats(request: DashboardRequest):
    ...
    try:
        ...
        # Safely filter and validate leads
        valid_leads = []
        for l in leads:
            try:
                # Try to validate key fields
                _ = l.get('lead_id', '')
                valid_leads.append(l)
            except:
                pass
        
        leads = valid_leads if valid_leads else leads
    ...
    try:
        ...
        if role == 'TELECALLER':
            ...
            for e in tele_activity:
                try:
                    lid = e.get('lead_id')
                    time = parse_datetime_safe(e.get('tele_actual_time', e.get('created_at')))
                    
                    if lid not in latest_by_lead:
                        latest_by_lead[lid] = e
                    else:
                        prev_time = parse_datetime_safe(latest_by_lead[lid].get('tele_actual_time', latest_by_lead[lid].get('created_at')))
                        if time > prev_time:
                            latest_by_lead[lid] = e
                except Exception as e:
                    print(f"Warning: Error comparing dates: {str(e)}")
                    # If error, just keep the first one
                    lid = e.get('lead_id')
                    if lid not in latest_by_lead:
                        latest_by_lead[lid] = e
            ...
            for m in meetings:
                try:
                    lid = m.get('lead_id')
                    time = parse_datetime_safe(m.get('created_at'))
                    
                    if lid not in latest_by_lead:
                        latest_by_lead[lid] = m
                    else:
                        prev_time = parse_datetime_safe(latest_by_lead[lid].get('created_at'))
                        if time > prev_time:
                            latest_by_lead[lid] = m
                except Exception as e:
                    print(f"Warning: Error comparing meeting dates: {str(e)}")
                    # If error, just keep the first one
                    lid = m.get('lead_id')
                    if lid not in latest_by_lead:
                        latest_by_lead[lid] = m
